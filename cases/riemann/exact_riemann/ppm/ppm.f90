...
!
! original from bruce fryxell, december 1992
! tweaked by fxt 1998, 2000, 2006, 2016, 2019
...
! evolution loop
      do nstep = 1, nmax
       ...
       if (nstep .eq. 1 .or. mod(nstep,10) .eq. 0) write(6,'(i5,1p2e10.2)') nstep, t, dt
       ...
       call store
       call tstep
       if (nstep .eq. 1 .or. mod(nstep,20) .eq. 0) call output
       if (t .ge. tmax) exit
      enddo 
...
! copy a row of the grid into the working arrays, and apply the boundary conditions
...
! local variables
      integer            :: i
      integer, parameter :: imap = 0    ! sets remapping
...
! declare the pass
! imap is a logical to set remapping
...
! popular formats
05    format('output_',i3.3,'.dat')
10    format (i6, 2x, 1pe12.5, 2x, e10.3, 2x, 0pf9.6, 2x, i4//)
20    format (i4, 5(2x, 1pe11.4))


      write(string,05) nstep
      open (unit=3, file=string, status='unknown')
      write(3,'(a,1f7.4)') 'time =',t
      write(3,'(t2,a,t9,a,t22,a,t35,a,t48,a,t61,a)') 'cell','radius', 'density', 'velocity', 'pressure', 'energy'
      do i = 1, nr
